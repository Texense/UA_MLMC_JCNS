% MLMC_LIF_Current_Based_Syn_Final.m
...
function [Vs,I_Es,I_Is,spike,t,SpikeCount,V_Nm,SynIndex] = MLMC_LIF_Current_Based_Syn_Final(dt,T,V_0s,eventsN,fs,S,see,sei,sie,sii,NE,NI,...
                                                                                           I_E0,I_I0,...
                                                                                           g_L,WindowSlide) % WindowSlide is a 1*2 vector, giving winsize and winslide
  steps = floor(T/dt);
...
%    dV = (dt*(-Vs(:,ii)/g_L) +(fs.*R(:,ii)+SE*spike(:,ii)-SI*spike(:,ii))/g_L ).*Ref(:,ii);
  ...
  SpikeCount = SpikeCount_slide(t,spike,WindowSlide(1),WindowSlide(2));
